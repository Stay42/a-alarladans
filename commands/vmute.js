...
message.channel.send(reawEmbed.setDescription(`${client.emojis.cache.get(a.yes)} ${member} kullanıcısı ${message.author} tarafından **"${zaman}"** boyunca **"${sebep}"** sebebiyle sesli kanallarda mutelendi!`));
message.guild.channels.cache.get(a.muteLog).send(reawEmbed.setDescription(`${client.emojis.cache.get(a.yes)} ${member} kullanıcısı ${message.author} tarafından **"${zaman}"** boyunca **"${sebep}"** sebebiyle sesli kanallarda mutelendi!`));

setTimeout(() => {
member.voice.setMute(false).catch();
message.guild.channels.cache.get(a.muteLog).send(reawEmbed.setDescription(`${client.emojis.cache.get(a.yes)} ${member} kullanıcısının **vmute** cezası bitti!`));
}, ms(zaman))

db.set(`vmuteli.${member.id}`, "vmuteli");
db.add(`vmuteSayi.${member.id}`, 1);
db.add(`vmuteSayiYT.${message.author.id}`, 1)
db.push(`sicil.${member.id}`, {Ceza: "vmute", Yetkili: message.author.id, Sebep: sebep})
